...
- 统计卡片：一行 5 格等宽 flex，格间 1px 分隔线；数值 22px/600 起、**自适应收缩字号
  保证单行**（v4 `FitValue` 组件测量 scrollWidth/clientWidth，最低 12px，有
  ResizeObserver 时跟随宽度变化），数值行固定 30px 高使各卡标签（12px 灰）**同一高度**。
  第 2 格下方加一行小字「单日峰值 ＋ 单次请求峰值」，第 3 格加「另：最长单轮」。
- 热力图：列=周、格=天 flex 网格，每格 10×10px、圆角 2px、间距 3px；底部月份
  标签行（该列首个可见格与上一列月份不同时显示「M月」）。色阶 5 档（对数比）：
  `lv0 rgba(128,128,128,.14)`（空）、`lv1 #f6c9b3`、`lv2 #ef9f7d`、`lv3 #e5764c`、`lv4 #c74e24`。
- ~~tabs：每日/每周/累计~~（v4 移除，只保留每日视图）。**改为按日历年分页**：
  标题右侧「‹ 前一年 / N 年 / 后一年 ›」；一页=一个日历年（本年截到今天，
  越界与未来格隐藏）；**前一年无记录时不显示「‹ 前一年」**，已在本年不显示「后一年 ›」。
- 图例：少 □□□□□ 多；每格**自定义悬停提示**（v4，纯 CSS :hover，不依赖
  window/document；边缘列贴边对齐防裁剪）：「8月15日消耗了 27.3 万 Token ·
  34 次请求」，无消耗日显示「8月13日没有 Token 消耗」。
...
> **注意**：以下为 v3 历史快照。**v4 起以仓库 `client.js` 为唯一准绳**（该文件内容即
> `code.client` 函数体，整段复制即可）；v4 变更点见上方 §5 与 `progress.md` 第五节。
...
把本文件交给那里的 agent，或直接说「按 ~/dsh-token-usage-dashboard 仓库（README.md）部署插件」。
标准流程：
