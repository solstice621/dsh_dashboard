// ===== dsh-token-usage-dashboard · Host 半（bundle 版）=====
...
// 版本：v8（修复 bundle 启动时序竞争——「设置页加载失败 / 路由 404」根因）
// v8：inject 声明 webServer 硬依赖。include 树并行激活时，本插件的 apply 可能先于
//     dsh-host-webserver 执行 → ctx.get('webServer') 为 undefined → 路由注册被静默跳过
//     （探针实证：apply begin webServer=NO，启动完成后枚举 fiber ctx webServer=YES）。
//     inject: ['webServer'] 让 cordis 等待服务提供后再 apply，时序问题根治。
...
const name = 'dsh-token-usage-dashboard'
// webServer 是硬依赖：注册数据路由必须等服务就绪（inject 保证时序，避免启动竞争）
