...
## 二十五、v24 修复「热力图颜色无区分」（2026-08-15）✅ 已部署

> 部署完成：`pkg-34` / `run-34`（仅 client.js；host v6 不变）；`plugin.json` 已回填。
> bundle 版 lib/client.js 同步。

### 用户反馈

热力图颜色没有区分（v23 数据修复后活跃日数据齐全，但颜色看不出梯度）。

### 根因

色阶是纯对数 5 档：`r = log(v+1)/log(max+1)`。当存在单一大峰值日（8/15 = 194,818,371）时，
对数刻度把 4,856万（8/16）也压到 r=0.93 → 与峰值日同为 lv4——活跃日全部同色，
其余全是灰 → 看起来「无区分」。

### 修复

- `cellLevel` 改为**对数+线性混合 6 档**：`r = 0.55*log占比 + 0.45*线性占比`（对数为主处理偏斜、
  线性为辅拉开大值梯度）；阈值 0.85/0.6/0.38/0.18；
- 新增 `.tks-lv5{background:#a83a12}` 深砖红；
- 实测映射：1.95亿→lv5、4,856万→lv4、千万→lv3、百万→lv3、10万~1千→lv2、微量→lv1、0→灰；
  8/15 与 8/16 颜色明确不同 ✅

---

## 二十六、v25 色阶改为纯线性（2026-08-15）✅ 已部署

> 部署完成：`pkg-35` / `run-35`（仅 client.js；host v6 不变）；`plugin.json` 已回填。
> bundle 版 lib/client.js 同步（同源 v25）。

### 用户反馈

v24 混合刻度（对数 0.55 + 线性 0.45）虽然拉开大值梯度，但用户明确表示：**「我个人倾向于搞成线性的」**。

### 变更

- `cellLevel` 改为**纯线性 6 档**：`r = v/max`，每档宽度 0.2（0.8/0.6/0.4/0.2 阈值）；
- 颜色深浅与消耗量完全按比例对应：峰值日（r=1）→ lv5，v ≥ 60% → lv4，≥ 40% → lv3，≥ 20% → lv2，其余 >0 → lv1，0 → 灰；
- `.tks-lv0..lv5` 色板不变；
- 当前数据实测：8/15（194.8M）→ lv5，8/16（48.6M，r≈0.25）→ lv2，微量日 → lv1 —— 完全线性。

### 说明

线性刻度下若未来数据更偏斜（单峰远大于其他日），小值日会落回 lv1/lv2——这是线性映射的固有行为，符合用户偏好。

---

## 二十七、v26 持久化快照 + 增量同步（重启不扫盘不重建 · 删会话保留统计）（2026-08-16）✅ 已部署

> 部署完成：`pkg-37` / `run-37`（host v7 + client v25）；`plugin.json` 已回填。
> bundle 版 lib/index.js 同步 v7（路径经 process.env.HOME）。

### 用户诉求

dsh 服务停止/重启后不要全量扫盘重建；删除会话记录后，已有的统计仍然保留。

### 设计（先查清 DSH 平台能力再动手）

1. `sessionPersistence.readFrom(id, fromSeq)` —— 从水位线增量读存储后缀（SQLite 物理 seek；
   JSONL 解析全文件但只返回增量）→「不扫盘」的底子；
2. `fs` 服务（Service 目录）实测可用：动态插件 Host 半能写盘 ✅（绝对路径；`~` 不展开）；
3. `storageDomain`/`settings` 需要 zod schema，动态插件无 import 环境用不了 → 用 fs 直写 JSON。

### 实现（host v7）

- **快照**：聚合状态（days/sessions/modelStats/openTurns + 各计数器 + 每会话 lastSeq 水位线）
  序列化为 `~/.dsh/storages/token-stats/snapshot.json`；10s 周期 flush（timer 服务）+ 卸载前
  best-effort flush；写失败留脏标记下周期重试；version 字段 + 结构校验。
- **启动优先快照**：加载成功 → `ready=true` 立即出数（无扫描占位），随后对每个会话
  `readFrom(id, lastSeq+1)` 增量折叠（实时流水位线去重，不重不漏）；全新会话从头读。
- **回退**：无快照 / JSON 损坏 / 版本不符 → 走 v6 全量 backfill（liveFloor + force 补折）。
- **删除会话**：快照里有、listSessions 没有的会话直接跳过 → 历史统计保留，不随删除回退；
  想按现存日志重建用 rescan RPC 或删快照文件重启。
- **rescan RPC** 语义不变（全量重建 + 落盘）。

### 实测（双路径重启验证）

| 场景 | 操作 | 结果 |
|---|---|---|
| 首次部署 | run-37 后检查快照 | totalTokens 261,587,805；峰值日 8/15 = 194,818,371 与磁盘基准逐位一致 ✅ |
| 快照损坏回退 | 停 → 写坏快照 → 启动 | 自动全量重建，快照重写；峰值日仍逐位一致 ✅ |
| 快照加载 | 再停 → 启动（快照完好） | savedAt 推进（增量同步落盘）；总量 262,001,948 → 262,106,481 只增不减 ✅ |
